...
class Loader(object):
    def __init__(self, args):
        ...
        self._betas_fea = checkpoint['module.feature.betas']
    ...
    def decode_cell(self):
        ...
        fea_bitstype = torch.argmax(self._betas_fea, dim=1)
        print(self._alphas_fea)
        print(self._betas_fea)
        return fea_genotype, fea_bitstype

def get_new_network_cell():
    args = obtain_decode_args()
    for i in range(11):
        args.resume = './search-EXP-20230726-110259/epoch_' + str(i+28) + '.pt'
        load_model = Loader(args)
        fea_genotype, fea_bitstype = load_model.decode_cell()
        print('Feature Net cell structure:', fea_genotype)
        print('Feature Net cell bits structure:', fea_bitstype)
